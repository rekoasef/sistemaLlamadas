...
export default function AnaliticaPage() {
  ...
  // 1. 📊 Rendimiento por Puesto (Terminal)
  ...
  // 3. ⚠️ Top Concesionarios con Llamadas Perdidas (Fugas de clientes)
  const fugasPorConcesionario = useMemo(() => {
    const map: any = {}
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-12 pb-20 animate-in fade-in duration-1000 px-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-neutral-900/40 border border-neutral-800 p-10 rounded-[3rem] shadow-2xl backdrop-blur-md">
          <div className="flex items-center justify-between mb-10">
            ...
            <span className="text-[9px] bg-red-600/10 text-red-600 px-3 py-1 rounded-full font-black italic uppercase border border-red-600/20 text-center">En Vivo</span>
          </div>
              ...
              return (
                <div key={p.id} className="space-y-3">
                  ...
                  <div className="w-full bg-neutral-800 h-3 rounded-full overflow-hidden flex shadow-inner border border-neutral-700">
                    <div 
                      className={`h-full transition-all duration-1000 shadow-[0_0_15px_rgba(220,38,38,0.3)] ${porcentaje > 70 ? 'bg-green-500' : porcentaje > 40 ? 'bg-yellow-500' : 'bg-red-600'}`} 
        ...
        {/* GRÁFICO 2: MAPA DE CALOR DE TRÁFICO */}
          ...
          <div className="flex-1 flex items-end justify-between gap-1 h-64 border-b border-neutral-800 pb-2">
            {statsHorarias.map((count, i) => {
              ...
              return (
                <div 
                  key={i} 
                  className={`transition-all rounded-t-sm relative group flex-1 ${count === max ? 'bg-red-600' : 'bg-neutral-700 hover:bg-red-400'}`}
                  style={{ height: `${height}%` }}
                >
      ...
      {/* SECCIÓN NUEVA: TOP FUGAS POR CONCESIONARIO */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-neutral-900/40 border border-neutral-800 p-10 rounded-[3rem]">
          <div className="flex items-center gap-3 mb-10">
            <AlertTriangle className="text-red-600" />
            <h3 className="text-[10px] font-black uppercase tracking-widest text-white italic">Ranking de Incidencias (Llamadas Perdidas)</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
             {fugasPorConcesionario.map((f: any, idx) => (
               <div key={idx} className="flex items-center justify-between p-5 bg-black/40 border border-neutral-800 rounded-2xl group hover:border-red-600 transition-colors">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[8px] text-neutral-600 font-black uppercase">Perdidas</p>
                  </div>
               </div>
             ))}
          </div>
        ...
        {/* MÉTRICA DE PROMEDIO DE LLAMADAS POR SENTIDO */}
        <div className="bg-red-600/5 border border-red-600/20 p-10 rounded-[3rem] flex flex-col justify-center items-center text-center">
             ...
             <h4 className="text-white font-black italic uppercase text-3xl tracking-tighter leading-none mb-2">
                {llamadas.filter(l => l.tipo_llamada === 'ENTRANTE').length} vs {llamadas.filter(l => l.tipo_llamada === 'SALIENTE').length}
             </h4>
             <p className="text-[10px] text-neutral-500 font-black uppercase tracking-[0.2em] italic">
                Proporción Entrante vs Saliente
             </p>
             <div className="w-full bg-neutral-800 h-1.5 mt-6 rounded-full overflow-hidden">
                <div 
                  className="bg-blue-500 h-full" 
                  style={{ width: `${(llamadas.filter(l => l.tipo_llamada === 'ENTRANTE').length / llamadas.length) * 100}%` }} 
                />
             </div>
        </div>
      </div>
      ...
      <div className="bg-red-600 p-12 rounded-[3.5rem] flex flex-col lg:flex-row items-center justify-between gap-8 shadow-2xl shadow-red-600/30">
        <div className="text-center lg:text-left">
          ...
          <p className="text-white/80 text-[11px] font-bold uppercase tracking-[0.2em] italic">
            Recibí la auditoría de rendimiento Crucianelli en tu correo cada semana
          </p>
        </div>
