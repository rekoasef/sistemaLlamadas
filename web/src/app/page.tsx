...
export default function DashboardPage() {
  ...
  // =============================
  // FETCH DE DATOS (CON RELACIÓN DIRECTA)
  // =============================
  const fetchLlamadas = useCallback(async () => {
    try {
      ...
      const { data: calls } = await supabase
        ...
        .select(`
          *,
          concesionarios:concesionario_id (
            nombre
          )
        `)
  ...
  // =============================
  // REALTIME CORREGIDO (CON RE-FETCH)
  // =============================
  useEffect(() => {
    setMounted(true)
    fetchLlamadas()
    ...
    const channel = supabase
      .channel('llamadas-realtime-industrial')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'llamadas' },
        async (payload: any) => {
          // Cada vez que hay un cambio (INSERT o UPDATE), 
          // pedimos la fila completa con el nombre del concesionario
          ...
          if (rowFull) {
            setLlamadasRaw(prev => {
              const existe = prev.find(l => l.id === rowFull.id)
              if (existe) {
                return prev.map(l => l.id === rowFull.id ? rowFull : l)
              }
              return [rowFull, ...prev].slice(0, 100)
            })
          }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in duration-700 pb-20 px-4">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between border-b border-neutral-800 pb-8 gap-6 mt-10">
        ...
        <div className="flex flex-wrap items-center gap-4 bg-neutral-900/50 p-4 rounded-[2rem] border border-neutral-800">
          ...
          <div className="flex items-center gap-2 bg-black/40 px-4 py-2 rounded-xl border border-neutral-800">
            ...
            >
              ...
              {listaDispositivos.map(id => (
                <option key={id} value={id}>PUESTO {id}</option>
              ))}
            </select>
          </div>
      ...
      <div className="bg-neutral-900/30 border border-neutral-800 rounded-[3rem] overflow-hidden shadow-2xl backdrop-blur-md">
        <div className="px-10 py-8 border-b border-neutral-800/50 bg-neutral-900/50 flex items-center justify-between">
          <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-white flex items-center gap-3 italic">
            <Activity size={16} className="text-red-600" /> Registro de Actividad
          </h3>
                ...
                <tr key={ll.id} className="hover:bg-red-600/[0.03] transition-all group">
                  ...
                  <td className="px-10 py-6">
                    {ll.concesionarios?.nombre ? (
                      <div className="flex flex-col">
                        <span className="text-white font-black italic uppercase text-2xl group-hover:text-red-500 transition-colors leading-none">
                          {ll.concesionarios.nombre}
                        </span>
                        <span className="text-[11px] text-neutral-600 font-mono italic mt-1 uppercase tracking-tighter">
                          {ll.numero_telefono}
                        </span>
                      </div>
                    ) : (
                      <div className="flex items-center gap-4">
                        <span className="text-neutral-500 font-mono text-base italic">
                          {ll.numero_telefono}
                        </span>
                        <button
                          onClick={() => {
                          ...
                          className="flex items-center gap-2 px-4 py-2 bg-neutral-800 border border-neutral-700 rounded-xl text-red-600 hover:bg-red-600 hover:text-white transition-all text-[10px] font-black uppercase italic"
                        >
                  ...
                  <td className="px-10 py-6 text-right">
                    <div className="flex flex-col items-end leading-none">
                      ...
                      <div className="flex items-center gap-1 text-[10px] font-black text-neutral-600 uppercase italic mt-1">
                        <Monitor size={11} className="text-red-600" /> PUESTO {ll.dispositivo_id}
                      </div>
                    </div>
                  </td>
                </tr>
  ...
  return (
    <div className="bg-neutral-900/50 border border-neutral-800 p-10 rounded-[2.5rem] relative overflow-hidden group shadow-lg">
      ...
      <div className="flex items-baseline gap-3">
        ...
        {sub && <span className="text-neutral-600 text-[11px] font-bold uppercase italic tracking-widest">{sub}</span>}
      </div>
      {bar && (
